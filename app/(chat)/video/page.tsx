...
import { ASPECT_RATIOS, DEFAULT_ASPECT_RATIO, download, fmt, JOB_KEY, TIERS, type AspectRatio, type Tier } from "./video-config";
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch("/api/video", {
        ...
        body: JSON.stringify({ prompt: prompt.trim(), tier: tierId, length: effectiveSeconds, aspectRatio }),
      });
      const data = await res.json();
...
// END OF FILE - app/(chat)/video/page.tsx (v4 - format picker)
