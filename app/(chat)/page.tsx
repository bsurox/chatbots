...
export default function VoicePage() {
  ...
  // The layout broadcasts this signal on every route change. The sample
  // preview player lives outside the page, so it must be stopped by
  // signal - navigation caching can keep this page alive, which means
  // the unmount cleanup above is not guaranteed to run.
  useEffect(() => {
    const stop = () => {
      previewAudioRef.current?.pause();
      setPlayingPreview(null);
    };
    window.addEventListener("evo-stop-audio", stop);
    return () => window.removeEventListener("evo-stop-audio", stop);
  }, []);
...
// -----------------------------------------------------------
// END OF FILE - app/(chat)/voice/page.tsx (v3.2 - stop signal)
