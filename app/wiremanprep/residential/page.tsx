// FILE: app/wiremanprep/residential/page.tsx
"use client";
...
import {
  ...
  type WrDomainKey,
  type WrQuestion,
} from "@/lib/wiremanprep/rquestions";

// WiremanPrep RESIDENTIAL practice room (v1) - the Residential
...
// component by itself.
// Sized for THIS exam (NASCLA Residential Electrical
// Contractors trade exam, PSI CIB 6677): the timer runs 2 min
// 42 sec per question (270 minutes / 100 questions - same pace
// as the Master exam), the pass bar reads 70% (lower than the
// Master's 75), the subject tiles show the 10 Residential
// domains' real weights out of 100, and the free tier serves
// the fixed 10-question Residential sample (buildWrDemoSet - a
// rotating draw would leak the bank). Paid = the wr flag from
// access v2; the gate sells Residential Prep at $79 flat and
// its buy button carries ?product=wr so checkout v2 charges the
// right product. Tutor route v4 and attempts v2 both understand
// wr- ids. Deliberate picker, A-D chips, tutor thread, report
...
// seconds a question (same clock as the Master exam).
...
export default function WiremanResidentialPracticePage() {
  ...
  useEffect(() => {
    fetch("/wiremanprep/api/access")
      ...
      .then((data) => {
        if (data) setAccess({ loggedIn: Boolean(data.loggedIn), paid: Boolean(data.wr) });
  ...
  const [qs, setQs] = useState<WrQuestion[] | null>(null);
  ...
  function startRound() {
    ...
    if (!access?.paid) {
      ...
      set = buildWrPracticeSet(sel === "all" ? "all" : sel, count);
    }
    setQs(set);
  ...
  if (phase === "pick") {
    return (
      <div className="fq-wrap">
        ...
        <p className="fq-title">Residential practice</p>
        <p className="fq-hint">
        ...
        <div className="fq-lenrow">
          ...
          <p className="fq-timernote">
            {access !== null && !access.paid
              ? "Residential Prep feature - the 1:1 exam-pace clock comes with the course."
              : timerOn
          ...
          {access !== null && !access.paid ? (
            <p className="fp-tryhint">
              The free round is a fixed 10-question sample. Residential
              Prep unlocks all 153 questions with fresh shuffles every
              round.
            </p>
          ) : null}
          {showGate ? (
            <div className="fp-gate">
              <p className="fp-gateh">
                {gateSrc === "timer"
                  ? "The exam timer is a Residential Prep feature."
                  : "Longer rounds are a Residential Prep feature."}
              </p>
              ...
              <Link className="fp-gatebtn" href="/wiremanprep/buy?product=wr">
                Get Residential Prep - $79
              </Link>
            </div>
          ) : null}
        </div>
  ...
  return (
    <div className="fq-wrap">
      ...
      <span className="fq-chip">{getWrDomain(question.domain)?.name ?? question.domain}</span>
...
// ============================================================
// END OF FILE - app/wiremanprep/residential/page.tsx (v1 -
// Residential practice room: 2:42/question pace, 70% bar, wr
// gate at $79, fixed Residential free sample)
